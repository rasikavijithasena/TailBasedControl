...
def write_data(NumSys,R_value,version, avg_power, avg_state_cost, avg_action_cost,var_state, var_control, avg_aoi):
    ...
    f = open("calculatedInfo/LQR/stat"+path_unique+".txt", "a")
...
def save_data(systems_plant,u_cont, alpha_u, P_s, beta,version, R, NumSys ):
    ...
    with open(r'dump/LQR/state_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(systems_plant, f)

    with open(r'dump/LQR/action_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(u_cont, f)

    with open(r'dump/LQR/schedule_alpha_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(alpha_u, f)

    with open(r'dump/LQR/power_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(P_s, f)

    with open(r'dump/LQR/beta_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(beta, f)
    ...
    for n in range(N):
        ...
        for t in range(NoTrainSample, T):
            ...
            for sys in range(NumSys):
                ...
                Q_Pu[0, sys, n, t + 1] = max(Q_Pu[0, sys, n, t] - gamm_Pu[0, sys, n, t], 0) + np.dot(alpha_u[0, sys, n, t], P_s[0, sys, n, t])
                Q_Cu[0, sys, n, t + 1] = max(Q_Cu[0, sys, n, t] - alpha_u[0, sys, n, t], 0) + max(min(np.real(m_u_sup[0, n, t]), 1), 0);
