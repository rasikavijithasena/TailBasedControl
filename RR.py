...
def write_data(NumSys, R_value, version, avg_power, avg_state_cost, avg_action_cost,var_state, var_control, avg_aoi):
    ...
    f = open("calculatedInfo/RRTail/stat" + path_unique + ".txt", "a")
    ...
    f.write('\n' + 'average aoi of ' + str(unique) + ' : ' + str(avg_aoi))
    f.close()
...
def save_data(systems_plant,u_cont, alpha_u, P_s, beta,version, R, NumSys ):
    ...
    with open(r'dump/RRTail/state_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(systems_plant, f)

    with open(r'dump/RRTail/action_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(u_cont, f)

    with open(r'dump/RRTail/schedule_alpha_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(alpha_u, f)

    with open(r'dump/RRTail/power_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(P_s, f)

    with open(r'dump/RRTail/beta_data_'+unique+'.pkl', 'wb') as f:
        pickle.dump(beta, f)
        ...
        for t in range(T):
            for sys in range(NumSys):
                ...
                if t % NumSys == (sys):
                    ...
                    systems_est[:, sys, n, t] = np.dot(np.dot(Cxy[:, :, sys, n, t], np.linalg.inv(Cyy[:, :, sys, n, t])), sx)
                    ...
                    alpha_u[0, sys, n, t] = 0
                    beta[0, sys, n, t] = beta[0, sys, n, t-1] +1
